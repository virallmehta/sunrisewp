...
global $wp_query, $artooz_theme_options;
...
$pagination = "standard";
if ( isset( $artooz_theme_options['product_display_pagination'] ) ) {
    $pagination = $artooz_theme_options['product_display_pagination'];
}
?>
...
<a href="#" class="load-more-btn"><?php _e( 'Load More', SS_DOMAIN ); ?></a>
...
<?php } ?>
	<?php
		$pagination = paginate_links( apply_filters( 'woocommerce_pagination_args', array(
			...
			'prev_text'    => apply_filters( 'sf_pagination_prev_text', __( '<i class="fa fa-angle-left" aria-hidden="true"></i>', SS_DOMAIN ) ),
			'next_text'    => apply_filters( 'sf_pagination_next_text', __( '<i class="fa fa-angle-right" aria-hidden="true"></i>', SS_DOMAIN ) ),
			'type'         => 'list',
			'end_size'     => 3,
			'mid_size'     => 3
		) ) );
