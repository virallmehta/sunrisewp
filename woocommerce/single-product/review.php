		...
		<div id="comment-<?php comment_ID(); ?>" class="comment_container">
			...
			<div class="comment-details">
				<?php if ( $rating && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) : ?>
					...
					<div itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="star-rating clearfix" title="<?php echo sprintf( __( 'Rated %d out of 5', AR_DOMAIN ), $rating ) ?>">
						<span style="width:<?php echo ( $rating / 5 ) * 100; ?>%"><strong itemprop="ratingValue"><?php echo esc_attr($rating); ?></strong> <?php _e( 'out of 5', AR_DOMAIN ); ?></span>
					</div>
				...
				<time class="date" itemprop="datePublished" datetime="<?php echo get_comment_date( 'c' ); ?>"><?php echo get_comment_date( __( get_option( 'date_format' ), AR_DOMAIN ) ); ?></time>
			</div>
			...
			<div class="comment-text">
				...
				<?php if ( $comment->comment_approved == '0' ) : ?>
					...
					<p class="meta"><em><?php _e( 'Your comment is awaiting approval', AR_DOMAIN ); ?></em></p>
						...
						<?php
							if ( get_option( 'woocommerce_review_rating_verification_label' ) === 'yes' ) {
								if ( function_exists( 'wc_review_is_from_verified_owner' ) ) {
									...
									if ( $verified ) {
										echo '<em class="verified">(' . __( 'verified owner', AR_DOMAIN ) . ')</em> ';
									}
								}
							}
						?>
