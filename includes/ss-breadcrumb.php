 ...
 *
*/
class apress_bread_crumb{
	public function apress_breadcrumbs( $args = '' ) {

		$default = array(
			'type'				=> 'list',
			'home_label'		=> __( 'Home', 'apress' ),
			'search_label'		=> __( 'Search Result of "%s"', 'apress' ),
			'404_label'			=> __( '404 Not Found', 'apress' ),
			'category_label'	=> _x( '%s', 'category label', 'apress' ),
			'tag_label'			=> _x( '%s', 'tag label', 'apress' ),
			'taxonomy_label'	=> _x( '%s', 'taxonomy label', 'apress' ),
			'author_label'		=> _x( '%s', 'author label', 'apress' ),
			'attachment_label'	=> _x( '%s', 'attachment label', 'apress' ),
			'year_label'		=> _x( '%s', 'year label', 'apress' ),
			'month_label'		=> _x( '%s', 'month label', 'apress' ),
			'day_label'			=> _x( '%s', 'day label', 'apress' ),
			'post_type_label'	=> _x( '%s', 'post type label', 'apress' ),
			'joint_string'		=> __( '&gt; ', 'apress' ),
			'navi_element'		=> '',
			'elm_class'			=> '',
			'elm_id'			=> 'crumbs',
			'li_class'			=> '',
		...
		if ( $args['type'] == 'string' ) {
			...
			foreach ( $bread_crumb_arr as $ancestor ) {
				if ( $cnt == count( $bread_crumb_arr ) ) {
					$output[] = '<strong class="' . esc_attr($args['current_class']) . '">' . apply_filters( 'the_title', $ancestor['title'] ) . '</strong>';
				} else {
					$output[] = '<a href="' . esc_url($ancestor['link']) . '">' . apply_filters( 'the_title', $ancestor['title'] ) . '</a>';
				}
				$cnt++;
			}
			...
			foreach ( $bread_crumb_arr as $ancestor ) {
				...
				if ( $cnt == count( $bread_crumb_arr ) ) {
					...
					$output .= $elm_tabs . $tabs . '	<li class="' . esc_attr(implode( ' ', $classes ));
					if ( $args['li_class'] ) {
					...
					$output .= '"><a href="' . esc_url($ancestor['link']) . '">' . apply_filters( 'the_title', $ancestor['title'] ) . '</a></li>&nbsp;/&nbsp;' . "\n";
				}
				$cnt++;
			}
			$output = apply_filters( 'bread_crumb_after', $output, $args );
	...
	private function get_bread_crumb_array( $args ) {
		...
		$bread_crumb_arr[] = array( 'title' => $args['home_label'], 'link' => esc_url(home_url('/')) );
		...
		if ( is_404() ) {
			...
			$bread_crumb_arr[] = array( 'title' => $parent_post->post_title, 'link' => esc_url(get_permalink( $parent_post->ID )) );
			$bread_crumb_arr[] = array( 'title' => sprintf( $args['attachment_label'], $post->post_title ), 'link' => esc_url(get_permalink( $post->ID )) );
		} elseif ( is_singular() && ! is_front_page() ) {
			$singular_bread_crumb_arr = $this->get_singular_bread_crumb_array( $post, $args );
			$bread_crumb_arr = array_merge( $bread_crumb_arr, $singular_bread_crumb_arr );
			$bread_crumb_arr[] = array( 'title' => $post->post_title, 'link' => esc_url(get_permalink( $post->ID ) ));
		} elseif ( is_category() ) {
			global $cat;
				...
				foreach( $ancestors as $ancestor ) {
					foreach ( $ancestor_posts as $ancestor_post ) {
						if ( $ancestor == $ancestor_post->ID ) {
							$bread_crumb_arr[] = array( 'title' => apply_filters( 'the_title', $ancestor_post->post_title ), 'link' => esc_url( get_permalink( $ancestor_post->ID ) ) );
						}
					}
				}
			}
	...
	private function add_posts_page_array( $bread_crumb_arr ) {
		...
		if ( get_option( 'show_on_front' ) == 'page' && $posts_page_id = get_option( 'page_for_posts' ) ) {
			...
			$bread_crumb_arr[] = array( 'title' => $posts_page->post_title, 'link' => esc_url(get_permalink( $posts_page->ID ) ));
		}
		return $bread_crumb_arr;
	}
...
$apress_bread_crumb = new apress_bread_crumb();

if ( ! function_exists( 'ss_breadcrumbs' ) ) {
	function ss_breadcrumbs( $args = '' ) {
		global $apress_bread_crumb, $post, $ss_theme_options;
		//return $apress_bread_crumb->apress_breadcrumbs( $args );
		echo 'bread';
		
		$remove_breadcrumbs = false;
		$breadcrumb_in_heading = 0;
		
		if ( isset( $ss_theme_options['breadcrumb_in_heading'] ) ) {
			$breadcrumb_in_heading = $ss_theme_options['breadcrumb_in_heading'];
		}

		// Remove breadcrumbs
        if ( $post && is_singular() ) {
            $remove_breadcrumbs = ss_get_post_meta( $post->ID, 'ss_no_breadcrumbs', true );
        }
        ss_theme_debug( $remove_breadcrumbs );
        ...
        if ( ! $remove_breadcrumbs ) {
        	echo $apress_bread_crumb->apress_breadcrumbs( $args );
        }


	}
	add_action( 'ss_main_container_start', 'ss_breadcrumbs', 5 );
}
/* Breadcumb End */
